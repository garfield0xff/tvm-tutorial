...
def create_relax_model():
    ...
    with bb.function("conv_compute"):
        ...
        with bb.dataflow():
            ...
            # Add bias: reshape bias to (1, out_channels, 1, 1) for broadcasting
...
if __name__ == "__main__":
    ...
    ex = relax.build(relax_model, target)
    vm = relax.VirtualMachine(ex, tvm.cpu())

    # Sampling input data
    x = tvm.runtime.tensor(np.random.rand(1, 3, 224, 224).astype("float32"))
    weight = tvm.runtime.tensor(np.random.rand(16, 3, 3, 3).astype("float32"))
    bias = tvm.runtime.tensor(np.random.rand(16).astype("float32"))

    result = vm["conv_compute"](x, weight, bias)
    result_np = result.numpy()
    
    print(f"result shape = {result_np.shape}")
    print(f"result min = {result_np.min()}")
    print(f"result max = {result_np.max()}")
    print(f"result mean = {result_np.mean()}")
    print(f"result std = {result_np.std()}")
